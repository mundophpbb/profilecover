...
class main_listener implements EventSubscriberInterface
{
    ...
    public static function getSubscribedEvents()
    {
        return [
            ...
            'core.acp_users_modify_profile' => 'assign_acp_template_vars',
            'core.acp_users_profile_validate' => 'validate_acp_cover_upload',
            'core.acp_users_profile_modify_sql_ary' => 'handle_acp_cover_upload',
    ...
    {
        $this->validate_uploaded_cover($event);
    }

    public function handle_cover_upload($event)
    {
        $this->handle_cover_save_for_row($event, $this->user->data);
    }
    ...
    public function assign_acp_template_vars($event)
    {
        $this->ensure_language_loaded();
        $this->ensure_cover_directory();
        $this->assign_asset_template_vars();

        $user_row = isset($event['user_row']) && is_array($event['user_row']) ? $event['user_row'] : [];
        $cover = !empty($user_row['user_profile_cover']) ? $user_row['user_profile_cover'] : '';
        $pos_x = $this->get_requested_cover_pos('profile_cover_pos_x', $this->get_user_cover_pos_x($user_row));
        $pos_y = $this->get_requested_cover_pos('profile_cover_pos_y', $this->get_user_cover_pos_y($user_row));

        $this->template->assign_var('S_FORM_ENCTYPE', ' enctype="multipart/form-data"');
        $this->assign_current_cover_to_template($cover, $pos_x, $pos_y);
        $this->assign_constraint_template_vars();
        $this->template->assign_var('S_PROFILE_COVER_DELETE_REQUESTED', (bool) $this->request->variable('delete_profile_cover', 0));
    }

    public function validate_acp_cover_upload($event)
    {
        $this->validate_uploaded_cover($event);
    }

    public function handle_acp_cover_upload($event)
    {
        $this->handle_cover_save_for_row($event, isset($event['user_row']) ? $event['user_row'] : []);
    }



    protected function validate_uploaded_cover($event)
    {
        $this->ensure_language_loaded();

        if (!$this->is_profile_form_submitted())
        {
            return;
        }
    ...
    protected function handle_cover_save_for_row($event, $row)
    {
        $this->ensure_language_loaded();
        $this->ensure_cover_directory();
        ...
        $target_user_id = isset($row['user_id']) ? (int) $row['user_id'] : (int) $this->user->data['user_id'];
        $current_cover = !empty($row['user_profile_cover']) ? $row['user_profile_cover'] : '';
        $current_pos_x = $this->get_user_cover_pos_x($row);
        $current_pos_y = $this->get_user_cover_pos_y($row);
        $new_pos_x = $this->get_requested_cover_pos('profile_cover_pos_x', $current_pos_x);
        $new_pos_y = $this->get_requested_cover_pos('profile_cover_pos_y', $current_pos_y);
        ...
        if ($upload['has_file'] && $upload['error'] === UPLOAD_ERR_OK)
        {
            ...
            $saved_filename = $this->process_and_store_cover($upload['tmp_name'], $extension, $new_pos_x, $new_pos_y, $target_user_id);
            if (!$saved_filename)
            {
                return;
            }
    ...
    protected function is_profile_form_submitted()
    {
        return $this->request->is_set_post('submit') || $this->request->is_set_post('update');
    }
    ...
    protected function get_requested_cover_pos($key, $default = self::DEFAULT_FOCAL_POS)
    {
        return $this->clamp_cover_pos((int) $this->request->variable($key, (int) $default));
    }
    ...
    protected function process_and_store_cover($tmp_name, $extension, $pos_x, $pos_y, $target_user_id = 0)
    {
        ...
        $owner_user_id = ($target_user_id > 0) ? (int) $target_user_id : (int) $this->user->data['user_id'];
        $filename = sprintf('cover_%d_%s.%s', $owner_user_id, $this->generate_token(), $extension);
